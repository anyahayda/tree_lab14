...
class BusyPossitionExcpetion(Exception):
...
class Game:
    def __init__(self):
        ...
        self.root_game_node = node.Node()
        node.build_tree_recursively(self.root_game_node)
                ...
                while error:
                    try:
                        ...
                        if pos in valid_turn_positions:
                            ...
                            raise BusyPossitionExcpetion
                    except OutOfRangePositionException:
                        print('Out of range position. Please enter free position from (0,0) to (2,2)')
                    except BusyPossitionExcpetion:
                        print('Busy position. Please enter free position')
                    except ValueError:
                        print('Invalid position format. Please enter two numbers(1 2)')
            else:
                best_turn_pos = None
